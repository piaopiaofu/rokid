...
public class UserService extends CommonService implements IUserService {
	...
	@Override
	public ResponseMap loginOut(RequestMap reqMap) {
		// 返回MAP对象
		ResponseMap resMap = new ResponseMap();
		...
		try {
			...
			if(sessionUserType != null && sessionUserType == 2){
				voiceTriggerDao.loginOutDelWorkDate(sessionUserId);
				asrNlpDao.loginOutDelWorkDate(sessionUserId);
			}
			// 成功返回
			resMap.setSuccessReturn();
	...
	public ResponseMap delete(RequestMap reqMap) {
		...
		try {
			...
			//ret = workMapper.deleteByUserId(id);
			
			//ret = workUserMapper.deleteByUserId(id);
			// 成功返回
			resMap.setSuccessReturn();				
		// 业务异常
		}catch(CannotGetJdbcConnectionException e){
